...
#define CALL_PHP \
if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_PI],colors[HC_PI+1]); \
...
/*!re2c
  ...
  STAGO name ws
  {
    ...
    commentstart=yytok;
    if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_OPENTAG],colors[HC_OPENTAG+1]);
    goto colorize_opentag;
  }
  STAGO TAGC
  { if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_ERROR],colors[HC_ERROR+1]); goto colorize_clear; }
  /*close tag*/
  ETAGO name ws
  {
    state[0]=PARSER_CLOSETAG;
    commentstart=yytok;
    if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_CLOSETAG],colors[HC_CLOSETAG+1]);
  ...
  MDO name ws
  {
    if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_MARKUP],colors[HC_MARKUP+1]);
  ...
  { if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_REFERENCE],colors[HC_REFERENCE+1]); goto colorize_clear; }
...
/*!re2c
  ...
  name (s)* "=" ws
  {
    if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_ATTRNAME],colors[HC_ATTRNAME+1]);
  ...
  (s)* "=" ws
  {
    ...
    {
      if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_ERROR],colors[HC_ERROR+1]);
      goto colorize_opentag;
    }
  }
  /*work around end*/
...
/*!re2c
  MSC MDC
  {
    if(lColorize) Info.pAddColor(lno,commentstart-line,yycur-commentstart,colors[HC_SUBSET],colors[HC_SUBSET+1]);
    state[0]=PARSER_CLEAR;
    goto colorize_clear;
  }
  DSC
...
/*!re2c
  PIC
  {
    if(lColorize) Info.pAddColor(lno,commentstart-line,yycur-commentstart,colors[HC_PI],colors[HC_PI+1]);
    state[0]=PARSER_CLEAR;
    goto colorize_clear;
  }
  [\000]
...
/*!re2c
  ...
  name0token ws
  {
    if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_ATTRVALUE],colors[HC_ATTRVALUE+1]);
    state[0]=PARSER_OPENTAG;
    goto colorize_opentag;
  }
  "\""
...
/*!re2c
  "\"" ws
  {
    if(lColorize) Info.pAddColor(lno,commentstart-line,yycur-commentstart,colors[HC_ATTRVALUE],colors[HC_ATTRVALUE+1]);
    state[0]=PARSER_OPENTAG;
    goto colorize_opentag;
  }
  /*php*/
  PIO PHP
  {
    if(lColorize) Info.pAddColor(lno,commentstart-line,yytok-commentstart,colors[HC_ATTRVALUE],colors[HC_ATTRVALUE+1]);
    CALL_PHP
  }
  [\000]
...
colorize_end:
    ...
    if(state[0]==PARSER_SUBSET)
      if(lColorize) Info.pAddColor(lno,commentstart-line,yyend-commentstart,colors[HC_SUBSET],colors[HC_SUBSET+1]);
    if(state[0]==PARSER_PI)
      if(lColorize) Info.pAddColor(lno,commentstart-line,yyend-commentstart,colors[HC_PI],colors[HC_PI+1]);
    if((state[0]==PARSER_STRING1)||(state[0]==PARSER_STRING2))
      if(lColorize) Info.pAddColor(lno,commentstart-line,yyend-commentstart,colors[HC_ATTRVALUE],colors[HC_ATTRVALUE+1]);
  }
colorize_exit:
