...
//this routines don's support file sizes more then 4G
...
bool FileEOF(HANDLE file)
{
  bool res=true;
  DWORD pos=SetFilePointer(file,0,NULL,FILE_CURRENT); BY_HANDLE_FILE_INFORMATION info;
  if(pos!=INVALID_SET_FILE_POINTER)
    if(GetFileInformationByHandle(file,&info))
    {
      if(info.nFileSizeLow>pos) res=false;
    }
  return res;
}
