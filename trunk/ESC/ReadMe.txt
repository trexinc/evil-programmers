...
Параметр    : value
Тип         : Строка
Описание    : Запрет   открытия  файла  сторонними  процессами.  Физически  это
              сделано следующим  образом:  при  открытии  или  перерисовке файл
              открывается  плагином  на  чтение  (GENERIC_READ) в  эксклюзивном
              режиме.  При  закрытии  файла  FAR'ом  или  при его сохранении он
              закрывается и плагином.
              Может быть: on - включить, off - выключить.
По умолчанию: off
Наследуется : Да
-------------- Какие параметры тега lockfile можно использовать <--------------
...
7. Дополнительные возможности для сторонних разработчиков
   ...
   Модуль esc.dll экспортирует функцию GetEditorSettingsW,  при  помощи  которой
...
bom
   ...
   Плагин  экспортирует  функцию  SetEditorOptionW,  при  помощи  которой  можно
изменить настройки  [ESC]  для  определенного  редактируемого  файла.  Прототип
функции:

int WINAPI SetEditorOptionW(
  intptr_t EditorID,
  const wchar_t *szName,
  void *Param
);
...
описание параметров GetEditorSettingsW).
...
макроса.
   ...
   Macro - указатель на HANDLE макроса, сам этот указатель не может быть  NULL.
           ...
           DeleteMacroW) и компиляция макроса. Если HANDLE  не  равен  NULL,  то
...
BOOL WINAPI CreateMultiMacro(
  ...
  DWORD Total
   ...
   Полный аналог CreateMacroW только Sequence в создаваемом макросе  повторяется
не один, а Total раз.

BOOL WINAPI CloneMacroW(
  const HANDLE Src,
  HANDLE *Dest
);
...
BOOL WINAPI IsMacroOKW(
  const HANDLE Macro
);
   Проверка макроса на корректность. Если макрос некорректен,  то  использовать
его (PostMacroW) не имеет смысла. В случае удачи возвращается число, отличное от
нуля.

BOOL WINAPI PostMacroW(
  const HANDLE Macro,
  BOOL silent
);
