...
/*!re2c
  ...
  "*/"
  {
    if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_RE2C],colors[HC_RE2C+1]);
    callback_data.row=lno;
  ...
  [()|=;/\\*+?]
  {
    if(lColorize) Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_KEYWORD],colors[HC_KEYWORD+1]);
    goto colorize_clear;
  }
  letter (letter|digit)*
  {
