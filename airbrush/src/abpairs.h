...
{ \
  ...
  if(flag) \
    Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_HIGHLITE],colors[HC_HIGHLITE+1]); \
  else \
    Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_KEYWORD1],colors[HC_KEYWORD1+1]); \
} \
goto colorize_clear;
...
{ \
  ...
  { \
    ...
    if(hl_state->flag) \
      Info.pAddColor(lno,yytok-line,yycur-yytok,colors[HC_HIGHLITE+err*2],colors[HC_HIGHLITE+1+err*2]); \
    if(flag) \
      Info.pAddColor(hl_state->row,hl_state->col,hl_state->len,colors[HC_HIGHLITE+err*2],colors[HC_HIGHLITE+1+err*2]); \
  } \
